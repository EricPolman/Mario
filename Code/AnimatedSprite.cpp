...
void AnimatedSprite::Update(float a_fDeltaTime){
	if(m_bIsVisible){
		if(m_bIsAnimated){
			...
			if(m_fAnimationTimer > 1 / data.speed / 4){
				m_fAnimationTimer = 0;
				Animate();
			}
		}
...
void AnimatedSprite::AddAnimation(std::string name, Vector2i beginFrame, Vector2i endFrame, float a_fAnimationSpeed){
	AnimationData data;
	data.begin = beginFrame;
	data.end = endFrame;
	data.speed = a_fAnimationSpeed == -1 ? m_fAnimationSpeed : a_fAnimationSpeed;
